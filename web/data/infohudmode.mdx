...
## Horizontal Speed
Displays the horizontal speed combined with momentum in pixels and 1/16th pixels (subpixels).
...
Also, if the number of frames is 7-9 (a delayed walljump) and the walljump is taking place in WRITG or Bubble Mountain or Mt. Everest, then additional information may be presented if you appear to be making hi-jump-less walljump attempts (also known as the hypo jump):
&nbsp;  
First two characters = Feedback on the delayed walljump:
- H# indicates you jumped high where the number tells you how many pixels you were high (can be zero indicating your height subpixel was too high and you will bonk).
- YH indicates you jumped at the optimum height with an 8 or 9 frame walljump.
- BH indicates you jumped at the optimum height with a 7 frame walljump which will bonk.
...
- BY indicates you jumped just below the optimum height with a 7 frame walljump which will bonk (PAL only since PAL physics allow for a larger range of heights to work).
...
- BL indicates you jumped below optimum height with a 7 frame walljump which will bonk.
...
## Double Springball Jump
Provides feedback on attempts to double springball jump:
&nbsp;  
First Y = You are on the ground with springball unequipped.
&nbsp;  
First two characters = Feedback on timing of morphing before the pause menu opens:
- Y# indicates you morphed in time where the number tells you how many frames remained before the menu opened (Y1 is the last possible frame).
- YL indicates you did not morph in time.

Last two characters = Feedback on subsequent pauses:
- E# indicates you pressed pause early where the number tells you how many frames you were early.
- Y# indicates the game should accept the pause input where the number tells you how many frames passed before the pause input (Y1 is the first possible frame).

## Boss Damage Counter
Counts the amount of damage dealt to a boss.

## Boss HP Counter
Counts the amount of HP remaining for a boss.
...
## Arm Pump Counter
Displays the number of consecutive single-frame armpumps. This is shown as a rolling count in order to have time to stop pumping and observe when a larger number is achieved.

## X Position
Displays the X position in hexadecimal pixels. Also Samus HP is replaced by the subpixels in hexadecimal.

## Y Position
Displays the Y position in hexadecimal pixels. Also Samus HP is replaced by the subpixels in hexadecimal.

## Camera Position
Displays the camera Layer 1 X and Y positions in hexadecimal pixels (overwriting Samus HP).

## Shot Timer
Displays the number of frames between pressing the shoot button.
Also the segment timer is overwritten by a set of four numbers representing the previous four shot timer values.
&nbsp;  
...
## Custom RAM Watch
Displays the value of two configurable memory addresses in any bank. Values can also be "locked" or rewritten each frame while the RAM Watch HUD mode is enabled. These addresses and values can be configured in the 'Customize RAM Watch' submenu.

# Room strat features
## Super HUD
Options to replace the item% and status icons and reserves area with other infohud modes, while still having a primary infohud mode or room strat. Compatible with most but not all other infohud modes and room strats.

## Ceres Ridley Hits
Display number of hits Ceres Ridley has taken. Ceres Ridley fly away conditions are upon Samus health dropping below 30 or upon hitting Ridley 100 times, at which point Ridley drops the baby metroid before recollecting it and flying away.

## Parlor-Botwoon Door Skip
Provides feedback on attempts to get out of bounds while falling through the door from parlor to climb or pre-botwoon to aqueduct:  
&nbsp;  
...
&nbsp;  
First two characters = Feedback on timing the first jump after pausing (except for botwoon on NTSC you actually pause before jump):
...
## Pit Room Right Door
Yes or No indicator on whether or not this flashing gray door has been opened.
...
## Kraid Nail Radar
Feedback on both Samus position and on Kraid's fingernails.
For Samus position, if you successfully perform the bomb bounce setup for KQK, then the Samus indicator will turn green.
...
## Wasteland Entry (currently only supported on NTSC)
...
## Kihunter Manipulation
Indicators on how the Kihunters are moving in the Red Kihunter Shaft, with Yes/No indicators on whether or not they are in a good position in order to travel from the bottom to the top of the room.
...
## Zeb Skip Indicator
Indicators useful for Zeb Skip. The yellow indicator is replaced by an I-Frame counter when Samus takes damage, since I-Frames are needed to get past a zebitite. When the green indicator appears, it means that Samus has correctly moved far enough left where she is at least one pixel inside the zebitite.

## Mother Brain HP
Displays the HP of the second enemy. Most useful for the Mother Brain fight since Mother Brain uses the second enemy slot.

## Two Cries Standup (currently only supported on NTSC)
