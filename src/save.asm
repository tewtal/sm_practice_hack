...
pre_load_state:
{
  ...
  .tinystate
    ...
    TDC : STA $4312 ; src addr
    LDA !ram_last_save_state_type : CMP #$0002 : BNE .tinystate_continue
    LDA #$4000 : STA $4312 ; src addr
  .tinystate_continue
    LDA #$0077 : STA $4314 ; src bank
...
post_load_state:
{
  ...
  .tileviewerDone
    ...
    LDA !ram_last_save_state_type : BEQ .checkFull
    DEC : BEQ .check1stTiny
    DEC : BEQ .check2ndTiny
    BRK

  .check1stTiny
    LDA !ram_minimap : CMP !SRAM_1ST_SAVED_MINIMAP : BNE .reloadMinimap
    BRA .minimapDone

  .check2ndTiny
    LDA !ram_minimap : CMP !SRAM_2ND_SAVED_MINIMAP : BNE .reloadMinimap
    BRA .minimapDone
  ...
  .disableMinimap
    LDA !IH_BLANK
    STA !HUD_TILEMAP+$3C : STA !HUD_TILEMAP+$7C : STA !HUD_TILEMAP+$BC
    STA !HUD_TILEMAP+$3E : STA !HUD_TILEMAP+$7E : STA !HUD_TILEMAP+$BE
    STA !HUD_TILEMAP+$40 : STA !HUD_TILEMAP+$80 : STA !HUD_TILEMAP+$C0
    STA !HUD_TILEMAP+$42 : STA !HUD_TILEMAP+$82 : STA !HUD_TILEMAP+$C2
    STA !HUD_TILEMAP+$44 : STA !HUD_TILEMAP+$84 : STA !HUD_TILEMAP+$C4
  .minimapDone
...
save_state:
{
    ...
    TAX : TAY
    LDA !ram_last_save_state_type : BEQ .save_dma_regs
    DEC : BEQ .save_1st_tiny_dma
    DEC : BEQ .save_2nd_tiny_dma
    BRK
  ...
  .save_1st_tiny_dma
    ; Store DMA registers to SRAM
    LDA $4300,X : STA !SRAM_1ST_DMA_BANK,X
    INX
    INY : CPY #$0B : BNE .save_1st_tiny_dma
    CPX #$7B : BEQ .save_dma_regs_done
    TXA : CLC : ADC #$05 : TAX
    LDY #$00
    BRA .save_1st_tiny_dma

  .save_2nd_tiny_dma
    ; Store DMA registers to SRAM
    LDA $4300,X : STA !SRAM_2ND_DMA_BANK,X
    INX
    INY : CPY #$0B : BNE .save_2nd_tiny_dma
    CPX #$7B : BEQ .save_dma_regs_done
    TXA : CLC : ADC #$05 : TAX
    LDY #$00
    BRA .save_2nd_tiny_dma

  .save_dma_regs_done
    %ai16()
    LDA !ram_last_save_state_type : ASL : TAX
    LDA.l save_write_tables,X : TAX
...
save_write_tables:
    dw #save_full_table
    dw #save_1st_tiny_table
    dw #save_2nd_tiny_table
...
save_2nd_tiny_table:
    ...
    ; Copy VRAM segments, uses $756C00-$765FFF
    %vram_to_sram($7C00, $400,  $756C00)
    %vram_to_sram($9000, $1000, $757000)
...
save_return:
{
  ...
  .continue
    LDA !ram_last_save_state_type : BEQ .continue_full
    DEC : BEQ .continue_1st_tiny
...
load_write_tables:
    dw #load_full_table
    dw #load_1st_tiny_table
    dw #load_2nd_tiny_table
...
load_2nd_tiny_table:
    ...
    ; Copy VRAM segments, uses $756C00-$765FFF
    %sram_to_vram($7C00, $400,  $756C00)
    %sram_to_vram($9000, $1000, $757000)
...
load_return:
{
  ...
  .load_1st_tiny
    LDA !SRAM_1ST_SAVED_SP : TCS
    BRA .load_continue

  .load_2nd_tiny
    LDA !SRAM_2ND_SAVED_SP : TCS
    BRA .load_continue
  ...
  .load_continue
    ...
    LDA !ram_last_save_state_type : BEQ .not_paused
  ...
  .not_paused
    %ai8()
    LDX #$00 : TXY
    LDA !ram_last_save_state_type : BEQ .load_dma_regs
    DEC : BEQ .load_1st_tiny_dma
    DEC : BEQ .load_2nd_tiny_dma
    BRK
  ...
  .load_1st_tiny_dma
    ; Load DMA registers from SRAM
    LDA !SRAM_1ST_DMA_BANK,X : STA $4300,X
    INX
    INY : CPY #$0B : BNE .load_1st_tiny_dma
    CPX #$7B : BEQ .load_dma_regs_done
    TXA : CLC : ADC #$05 : TAX
    LDY #$00
    BRA .load_1st_tiny_dma

  .load_2nd_tiny_dma
    ; Load DMA registers from SRAM
    LDA !SRAM_2ND_DMA_BANK,X : STA $4300,X
    INX
    INY : CPY #$0B : BNE .load_2nd_tiny_dma
    CPX #$7B : BEQ .load_dma_regs_done
    TXA : CLC : ADC #$05 : TAX
    LDY #$00
    BRA .load_2nd_tiny_dma
