        ...
        with open(output_filepath, 'w', newline='\n') as outfile:
            ...
            for line in infile:
                if line.startswith("; "):
                    ...
                    safety_value = line[16:22]
                    preset_name = line[25:].split("\"")[0]
                    preset_label = compute_label_from_name(preset_name)
                    if preset_label in all_preset_names:
                        if preset_name != all_preset_names[preset_label]:
                            raise Exception("Name conflict: " + preset_name + " and " + all_preset_names[preset_label] + " both resolve to " + preset_label)
                    ...
                    outline = f'presets_{category_preset_label}:\n'
                    outfile.write(outline)
                    if safety_value.startswith("00"):
                        outline = f'    %cm_safety(${safety_value}, \"{preset_name}\", #preset_names_{preset_label}, !sram_safeties_enabled_{file_label})\n'
                    else:
                        outline = f'    %cm_safety(${safety_value}, \"{preset_name}\", #preset_names_{preset_label}, !sram_safeties_enabled_msb_{file_label})\n'
                else:
                    if outline:
                        outfile.write(outline)
                    outline = line
