...
CrashHandler:
{
  ...
  .saveStackSize
    ...
    LDA !ram_crash_type : AND #$4000 : BEQ .launchCrashViewer
...
CrashDump:
{
  ...
  .drawStackStartingOffsetLoop
    PHA : AND #$0007 : BEQ .drawStackStartingOffsetFound
    TXA : CLC : ADC #$0006 : TAX
    LDA !ram_crash_stack_line_position : INC : STA !ram_crash_stack_line_position
    PLA : INC : BRA .drawStackStartingOffsetLoop

  .drawStackStartingOffsetFound
    PLA : %a8()
...
CrashMemViewer:
{
  ...
  .pressedUp
    LDA !ram_crash_cursor : BNE .pressedUpDecrement
    LDA #$0003
  .pressedUpDecrement
  ...
  .pressedDown
    LDA !ram_crash_cursor : CMP #$0002 : BMI .pressedDownIncrement
    LDA #$FFFF
  .pressedDownIncrement
  ...
  .drawLowerHalfNearby
    ...
    ; inc tilemap position
    INX #6 : LDA !ram_crash_stack_line_position : INC
    STA !ram_crash_stack_line_position : AND #$08 : BEQ .drawLowerHalfNearbyInc
  ...
  .drawUpperHalfNearby
    ...
    ; inc tilemap position
    INX #6 : LDA !ram_crash_stack_line_position : INC
    STA !ram_crash_stack_line_position : AND #$08 : BEQ .drawUpperHalfNearbyInc
